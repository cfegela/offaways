...
graph TB
    ...
    subgraph "Frontend - S3"
        S3[S3 Bucket<br/>Static Files<br/>HTML/CSS/JS]
        OAC[Origin Access Control]
    end

    subgraph "API Gateway + Lambda"
        APIGW[API Gateway<br/>/api/*]
        Auth[Cognito Authorizer]

        subgraph "Lambda Functions"
            CognitoConfigFn[cognitoConfig]
            FilingFns[Filing Handlers<br/>list/create/get/update/delete/setStatus]
            SeriesFns[Series Handlers<br/>list/create/update/delete]
            VoteFns[Vote Handlers<br/>list/create/update/delete/import/clear]
            UserFns[Admin User Handlers<br/>list/create/get/update/delete]
        end
    end

    subgraph "Authentication"
        Cognito[Cognito User Pool]
        AdminsGroup[Admins Group]
    ...
    subgraph "VPC - us-east-1"
        subgraph "Private Subnets"
            ...
            AuroraSG[Aurora Security Group<br/>Port 5432]

            subgraph "Aurora Serverless v2"
                Cluster[Aurora Cluster<br/>PostgreSQL 16.6<br/>Min: 0.5 ACU<br/>Max: 8 ACU]
                Instance[Aurora Instance<br/>db.serverless]
            end
        end
    end

    subgraph "Parameter Store"
        SSM[SSM Parameter<br/>/offaways/stage/db-password]
    end
    ...
    S3 -.->|OAC| OAC
    OAC -.->|Authorized Access| CF

    APIGW -->|Cognito JWT| Auth
    Auth -->|Validates Token| Cognito
    Cognito -.->|Group Membership| AdminsGroup

    APIGW --> CognitoConfigFn
    APIGW -->|Authorized| FilingFns
    APIGW -->|Authorized| SeriesFns
    APIGW -->|Authorized| VoteFns
    APIGW -->|Admin Only| UserFns

    CognitoConfigFn -.->|Returns Config| Cognito
    FilingFns -->|VPC| LambdaSG
    SeriesFns -->|VPC| LambdaSG
    VoteFns -->|VPC| LambdaSG
    UserFns -->|VPC + Cognito API| LambdaSG
    UserFns -.->|Admin Operations| Cognito

    LambdaSG -->|Port 5432| AuroraSG
    AuroraSG --> Cluster
    Cluster --> Instance

    FilingFns -.->|Reads Password| SSM
    SeriesFns -.->|Reads Password| SSM
    VoteFns -.->|Reads Password| SSM
    UserFns -.->|Reads Password| SSM
    ...
    style Cluster fill:#3b48cc
    style Instance fill:#3b48cc
    style SSM fill:#dd344c
    style LambdaSG fill:#ff9900
    style AuroraSG fill:#ff9900
```

**Key Components:**

- **CloudFront**: Single distribution serving both static frontend (S3) and API (API Gateway) with path-based routing (`/api/*` → API Gateway, everything else → S3)
- **S3 + OAC**: Frontend static files secured with Origin Access Control (no public access)
- **Cognito**: User Pool with email-based auth, password policy, and Admins group for role-based access
- **API Gateway**: REST API with Cognito authorizer validating JWT tokens on all protected endpoints
- **Lambda Functions**: 20+ handlers for filings, series, proxy votes, and admin user management
- **Aurora Serverless v2**: PostgreSQL cluster in VPC with auto-scaling (0.5–8 ACU), 7-day backups, snapshot on deletion
- **VPC Networking**: Lambda and Aurora isolated in private subnets across 3 AZs, security groups control Lambda→Aurora traffic on port 5432
- **SSM Parameter Store**: Secure storage for database password, accessed by Lambda at runtime
